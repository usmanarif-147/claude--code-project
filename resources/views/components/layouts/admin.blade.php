...
<body class="bg-dark-900 text-gray-300 font-sans antialiased" x-data="{ sidebarOpen: false }">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
        ...
        <div class="h-16 flex items-center px-6 border-b border-dark-700">
            <a href="/" class="text-xl font-bold text-white hover:text-accent-400 transition-colors">
                UA<span class="text-accent-400">.</span>
            </a>
        </div>
        ...
        <nav class="flex-1 px-4 py-6 space-y-1">
            ...
            <div x-data="{ portfolioOpen: {{ $portfolioActive ? 'true' : 'false' }} }">
                <button @click="portfolioOpen = !portfolioOpen"
                        class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium {{ $portfolioActive ? 'bg-accent-500/10 text-accent-400' : 'text-gray-400 hover:text-white hover:bg-dark-700' }} transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div x-show="portfolioOpen"
                    ...
                    <a href="{{ route('admin.profile.edit') }}"
                       class="flex items-center gap-3 pl-10 pr-3 py-2 rounded-lg text-sm font-medium {{ request()->routeIs('admin.profile.*') ? 'bg-accent-500/10 text-accent-400' : 'text-gray-400 hover:text-white hover:bg-dark-700' }} transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="{{ route('admin.technologies.index') }}"
                       class="flex items-center gap-3 pl-10 pr-3 py-2 rounded-lg text-sm font-medium {{ request()->routeIs('admin.technologies.*') ? 'bg-accent-500/10 text-accent-400' : 'text-gray-400 hover:text-white hover:bg-dark-700' }} transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="{{ route('admin.experiences.index') }}"
                       class="flex items-center gap-3 pl-10 pr-3 py-2 rounded-lg text-sm font-medium {{ request()->routeIs('admin.experiences.*') ? 'bg-accent-500/10 text-accent-400' : 'text-gray-400 hover:text-white hover:bg-dark-700' }} transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="px-4 py-4 border-t border-dark-700">
            <div class="flex items-center gap-3 px-3 py-2">
                <div class="w-9 h-9 rounded-full bg-accent-500 flex items-center justify-center text-white font-semibold text-sm">
    ...
    <div class="lg:hidden fixed top-0 left-0 right-0 z-30 h-16 bg-dark-800 border-b border-dark-700 flex items-center px-4">
        ...
        <span class="ml-4 text-lg font-bold text-white">UA<span class="text-accent-400">.</span></span>
    </div>
