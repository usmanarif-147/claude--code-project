<div>
    {{-- Header --}}
    <div class="mb-8">
        <h1 class="text-2xl font-bold text-white">Resume Generator</h1>
    ...
    <div class="bg-dark-800 border border-dark-700 rounded-xl p-6 mb-8">
        <h3 class="text-lg font-semibold text-white mb-3">Data Summary</h3>
    ...
    {{-- Template Selector --}}
    <div class="bg-dark-800 border border-dark-700 rounded-xl p-6 mb-8">
        <h3 class="text-lg font-semibold text-white mb-4">Choose Template</h3>
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            @foreach($templates as $key => $description)
                <button wire:click="$set('selectedTemplate', '{{ $key }}')"
                        class="p-4 rounded-xl border-2 text-left transition-all {{ $selectedTemplate === $key ? 'border-accent-500 bg-accent-500/10' : 'border-dark-700 bg-dark-700 hover:border-dark-600' }}">
    ...
    <div class="bg-dark-800 border border-dark-700 rounded-xl p-6 mb-8">
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="{{ route('admin.resume.download', $selectedTemplate) }}"
               class="inline-flex items-center gap-2 bg-accent-500 hover:bg-accent-600 text-white font-medium rounded-lg px-6 py-2.5 transition-colors text-sm">
