<div>
                            ...
                            <div class="flex flex-wrap gap-1">
                                @foreach($day['tasks']->take(3) as $task)
                                    <span class="w-1.5 h-1.5 rounded-full shrink-0"
                                          style="background-color: {{ $task->category ? $task->category->color : '#6b7280' }}"></span>
                                @endforeach
                            </div>
                    ...
                    <div wire:click="openDayModal('{{ $day['date'] }}')"
                        ...
                        @forelse($day['tasks'] as $task)
                            <div class="rounded-lg px-2 py-1.5 mb-1 text-xs {{ $task->status === 'completed' ? 'opacity-50' : '' }}"
                                 style="border-left: 3px solid {{ $task->category ? $task->category->color : '#7c3aed' }}; background-color: {{ $task->category ? $task->category->color.'1a' : 'rgba(124, 58, 237, 0.1)' }}">
                                <span class="{{ $task->status === 'completed' ? 'line-through text-gray-500' : 'text-gray-300' }} truncate block">
                                    {{ $task->title }}
                                </span>
                            </div>
                        @empty
    ...
    <div class="flex flex-wrap items-center gap-4 mt-4 px-2">
        ...
        @endforeach
        <div class="flex items-center gap-1.5">
            <span class="w-3 h-3 rounded-full shrink-0 bg-gray-500"></span>
