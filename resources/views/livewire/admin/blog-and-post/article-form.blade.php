<div>
    ...
    <div class="flex items-center gap-2 text-xs text-gray-500 mb-2">
        ...
        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
        <span class="text-gray-300">{{ $post?->exists ? 'Edit' : 'New' }} Article Post</span>
    </div>
    ...
    <div class="flex items-center justify-between mb-6 gap-4 flex-wrap">
        <div>
            <div class="flex items-center gap-3 flex-wrap">
                <span class="inline-flex items-center justify-center w-10 h-10 rounded-lg bg-blue-500/10 text-blue-400">
                ...
                <h1 class="text-2xl font-mono font-bold text-white uppercase tracking-wider">
                    {{ $post?->exists ? 'Edit Article Post' : 'New Article Post' }}
                </h1>
            </div>
            <p class="text-gray-500 mt-1">Share a URL — LinkedIn fetches the page's og: tags to build the preview card.</p>
        </div>
    ...
    <div class="bg-dark-800 border border-dark-700 rounded-xl p-6 space-y-6">
        ...
        <div>
            ...
            <input type="text" id="title" wire:model="title" maxlength="255"
                   placeholder="e.g. My new blog post on async PHP"
        ...
        <div>
            ...
            <textarea id="caption" wire:model="caption" rows="8"
                      placeholder="Write the post body that appears above the article preview..."
            ...
            <p class="mt-1 text-xs text-gray-500">Your commentary on the article — appears above LinkedIn's preview card.</p>
            @error('caption') <p class="mt-1 text-sm text-red-400">{{ $message }}</p> @enderror
        </div>

        {{-- Article URL --}}
        <div>
            <label for="article_url" class="block text-sm font-medium text-gray-300 mb-2">
                Article URL <span class="text-red-400">*</span>
            </label>
            <div class="relative">
                ...
                <input type="url" id="article_url" wire:model.live.debounce.500ms="article_url" maxlength="500"
                       placeholder="https://usmaniqbal.dev/blog/async-php-traps"
                       class="w-full bg-dark-700 border border-dark-600 rounded-lg pl-9 pr-4 py-2.5 text-white text-sm placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all duration-200">
            </div>
            <p class="mt-1 text-xs text-gray-500">
                LinkedIn will fetch this URL's <code class="text-primary-light">og:title</code>,
                <code class="text-primary-light">og:description</code>, and <code class="text-primary-light">og:image</code>
                to render a preview card. Make sure your page has those meta tags.
            </p>
            @error('article_url') <p class="mt-1 text-sm text-red-400">{{ $message }}</p> @enderror

            {{-- Fetch in-flight pill --}}
            <div wire:loading.flex wire:target="article_url"
                 class="mt-3 items-center gap-2 text-xs font-medium text-primary-light bg-primary/10 border border-primary/20 rounded-full px-3 py-1.5 w-fit">
                <svg class="w-3.5 h-3.5 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>
                Loading preview...
            </div>

            {{-- OG preview card --}}
            <div wire:loading.remove wire:target="article_url">
                @if ($ogPreview['ok'])
                    <div class="mt-3 bg-dark-700 border border-dark-600 rounded-lg p-3 flex gap-3">
                        @if (!empty($ogPreview['image']))
                            <img src="{{ $ogPreview['image'] }}" alt="OG image"
                                 class="w-20 h-20 object-cover rounded-lg shrink-0" loading="lazy"
                                 onerror="this.style.display='none'">
                        @endif
                        <div class="min-w-0 flex-1">
                            @if (!empty($ogPreview['title']))
                                <p class="text-sm font-semibold text-white truncate">{{ $ogPreview['title'] }}</p>
                            @endif
                            @if (!empty($ogPreview['description']))
                                <p class="text-xs text-gray-400 mt-1 line-clamp-2">{{ $ogPreview['description'] }}</p>
                            @endif
                            <p class="text-xs text-gray-500 mt-1 truncate">
                                {{ $ogPreview['site_name'] ?? parse_url($article_url, PHP_URL_HOST) }}
                            </p>
                        </div>
                    </div>
                @elseif (!empty($ogPreview['error']))
                    <div class="mt-3 flex items-start gap-2 bg-amber-500/5 border border-amber-500/20 text-amber-400 rounded-lg px-3 py-2 text-xs">
                        ...
                        <span>
                            Could not fetch preview: {{ $ogPreview['error'] }}. LinkedIn may still render it correctly &mdash; that depends on the target site's meta tags.
                        </span>
                    </div>
                @endif
            </div>
